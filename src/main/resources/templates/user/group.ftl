                    ...
                    <div class="box box-solid">
                        ...
                        <div class="box-footer" style="">
                            <form class="form-horizontal" onsubmit="return preForm();"
                                  action="${request.contextPath}/security/user/group" method="post">
                                <input name="id" value="${securityUser.id?c}" style="display: none;" readonly>
                                <input id="groupIds" name="groupIds" value="${groupIds!}"
...
<script>
    var datatable = $('#resultTable').DataTable({
        ...
        if ($('#groupIds').val().length > 0) {
            $.each($('#groupIds').val().split(','), function (index, item) {
